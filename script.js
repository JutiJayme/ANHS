...
// QR scanning for staff
function startQRScan() {
  QrScanner.scanImage({ facingMode: 'environment' })
    .then(result => {
      const student = users[result];
      if (student && student.type === 'student') {
        document.getElementById('scan-result').classList.remove('hidden');
        document.getElementById('student-id').textContent = `Student ID: ${student.id}`;
        const historyList = document.getElementById('usage-history');
        ...
        showError('Student not found or invalid QR.');
      }
    })
    .catch(() => showError('QR scan failed.'));
}
